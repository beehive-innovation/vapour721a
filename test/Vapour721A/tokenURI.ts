...
import {concat, eighteenZeros, getChild, op, Opcode} from "../utils";
...
describe("Vapour721A tokenURI test", () => {
	before(async () => {
		...
		await currency
			...
			.approve(
				vapour721A.address,
				ethers.BigNumber.from("100" + eighteenZeros)
			);
		await currency
			.connect(buyer1)
			.approve(
	...
	it("Should revert when calling tokenURI() for a non-existent id", async () => {
		await expect(
			vapour721A
				.connect(buyer0)
				.tokenURI((vapour721AConstructorConfig.supplyLimit as number) + 1)
	...
	it("Shouldn't enforce any format when setting baseURI", async () => {
		const baseURIs = [
			"http://google.com",
			"ipfs://QmPvjoUdAJVZMVACLqj77wsrQyaJVu9VpKdDRHvFK5cLzR",
			"Some other string",
		];

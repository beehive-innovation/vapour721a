...
describe("mintNFT tests", () => {
	...
	describe("supply limit tests", async () => {
		...
		it("should allow minting up to the supply limit", async () => {
			...
			await currency
				.connect(buyer1)
				.mintTokens(vapour721AConstructorConfig.supplyLimit);
			await currency
				.connect(buyer1)
				.approve(vapour721A.address, nftPrice.mul(units));
		...
		it("should fail to buy beyond supplyLimit even after NFTs have been burned", async () => {
			...
			expect(await vapour721A.balanceOf(buyer1.address)).to.equals(
				(vapour721AConstructorConfig.supplyLimit as number) - 1
			);
			expect(await vapour721A.totalSupply()).to.equals(
				(vapour721AConstructorConfig.supplyLimit as number) - 1
			);
	...
	describe("revert tests", () => {
		before(async () => {
			...
			await currency
				...
				.approve(
					vapour721A.address,
					nftPrice.mul(
						ethers.BigNumber.from(vapour721AConstructorConfig.supplyLimit)
					)
				);
		});
		...
		it("should revert if minimum units exceeds current stock", async () => {
			// buy half the stock
			const buyer1Units =
				(vapour721AConstructorConfig.supplyLimit as number) / 2;
			...
			const buyer2Units =
				(vapour721AConstructorConfig.supplyLimit as number) - buyer1Units + 1;

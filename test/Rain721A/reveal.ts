...
describe("Rain721A Reveal test", () => {
    ...
    it("Should fail to reveal with wrong secret",async () => {
        await expect(rain721a.connect(buyer0).reveal(secrets[1])).to.revertedWith("BAD_SECRET");
    });
    ...
    it("Should fail to reveal with same secret",async () => {
        await expect(rain721a.connect(buyer0).reveal(secrets[0])).to.revertedWith("BAD_SECRET");
    });
});